...
namespace SAF_Website
{
    public partial class AddNewQuestion : System.Web.UI.Page
    {
		...
		{
			//To verify the which user is logging in currently
		...
		{
			//Set the Textbox(title) readability
			TextBoxTitle.ReadOnly = true;
			TextBoxTitle2.ReadOnly = true;
			
			//Set the second edit button to false and first one to true
			LinkButtonEdit.Visible = true;
			LinkButtonEdit2.Visible = false;

			//Set the first textbox(title) to false and second textbox(title) to true
			TextBoxTitle2.Visible = true;
			TextBoxTitle.Visible = false;

			//Save the Textbox2 (title) into a session
				...
				if (response.IsSuccessStatusCode)
				{
					...
					if (results.Count == 0)
					{
						...
						using (var client2 = new HttpClient())
						{
							...
							//Set the readability of Textbox2(title) to true
							TextBoxTitle2.ReadOnly = true;

							//Set the visibility of the edits buttons and the msg label
							LinkButtonEdit.Visible = true;
							LinkButtonEdit2.Visible = false;
		...
		{
			//Check if the user selected that index and show what options are available to them
		...
		{
			//Set the readibility of Textbox2(title)
			TextBoxTitle2.ReadOnly = true;

			//Set the visibility of the edits,cancel buttons and the msg label 
			LinkButtonCancel.Visible = false;
			LinkButtonEdit.Visible = true;
			LinkButtonEdit2.Visible = false;
			theMsg.Visible = false;
			
			//Get the last checklist ID so if user were to cancel and didnt update it will return to the default checklist name
			using (var client3 = new HttpClient())
			{
			...
			//Set Textbox2(title) to the last checklist name which is the default
